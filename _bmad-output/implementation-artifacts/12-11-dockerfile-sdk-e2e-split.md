...
Status: done (runtime verification green after option (a) memvid-node Linux build was implemented inline; see 2026-04-15 Change Log)
ui_impact: false
epic: 12
story_id: 12-11
story_type: fix
...
| 2026-04-15 | **Runtime verification GREEN — option (a) implemented.** Added a `rust:1.88-alpine` `rust-builder` stage to `docker/Dockerfile.sdk-e2e` that compiles `packages/memvid-node` into a Linux musl `.node` addon. The sibling `memvid` Rust crate is supplied via BuildKit `--build-context memvid=...`; `scripts/sdk-e2e-infra.sh` now resolves `MEMVID_DIR` (default `../memvid`) and passes the context. After the rust stage, the node builder copies the `.node` + generated `index.{js,cjs}` into `packages/memvid-node/`, then `pnpm -r build` tsc-compiles pet-circuit and pet-dvm (memvid-node alone stays excluded from the filter since its build script is `napi build` and no Rust toolchain exists in the node stage). All three `@toon-protocol/*` workspace packages plus their transitive runtime deps (o1js + rot-js → added to the `/tmp npm install` so transitive deps like `cachedir` are flat-resolved) are staged into `/runtime/node_modules/`. **Runtime results:** both peers come up healthy (`curl :19100/health` + `:19110/health` return `status:"healthy"`); AC-11 PASS, AC-12 PASS (`down` → 0 containers), AC-13 partial (12 SDK E2E tests pass, 7 fail on pre-existing "Settlement infrastructure not enabled" / Solana confirmation timeout — not 12.11 regressions). Status: `blocked` → `done`. Image size: 592MB (vs pre-option-(a) 480MB; the +112MB is o1js + rot-js + the three @toon-protocol packages staged into /runtime). |
