...
- No conflicting E2E framework (no playwright.config.*, no cypress.config.*)
- Existing test framework: **Vitest ^1.0** already configured
  - Root vitest.config.ts (unit tests: packages/*/src/**/*.test.ts)
  - packages/client/vitest.config.ts (client unit tests)
  - packages/client/vitest.e2e.config.ts (E2E: tests/e2e/**/*.test.ts)
