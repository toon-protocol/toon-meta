...
- NFR-SDK-7: The SDK package size SHALL be minimal, depending only on `@crosstown/core`, `@crosstown/relay` (for TOON codec), `nostr-tools`, `@scure/bip39`, `@scure/bip32`
...
FR-NIP34-1:   Epic 3, Story 3.1 - Git HTTP backend and NIP-34 handler service node
FR-NIP34-2:   Epic 3, Story 3.2 - Nostr pubkey-native git identity
FR-NIP34-3:   Epic 3, Story 3.3 - Read-only code browsing web UI (TypeScript port)
FR-NIP34-4:   Epic 3, Story 3.4 - PR lifecycle via NIP-34 status events
FR-NIP34-5:   Epic 3, Story 3.5 - Issues/PRs from Nostr events on relay
FR-NIP34-6:   Epic 3, Story 3.6 - Publish @crosstown/rig package

## Epic List

### Epic 1: ILP-Gated Service Node SDK
A developer can create a working ILP-gated service node from a 12-word seed phrase in ~10 lines of code. The SDK provides unified secp256k1 identity (Nostr + EVM), TOON-native kind-based event handling with raw TOON passthrough for LLM consumption and lazy decode for code handlers, configurable pricing validation, embedded connector lifecycle management, network discovery, and dev mode. Published as `@crosstown/sdk`.
**FRs covered:** FR-SDK-1, FR-SDK-2, FR-SDK-3, FR-SDK-4, FR-SDK-5, FR-SDK-6, FR-SDK-7, FR-SDK-8, FR-SDK-9, FR-SDK-10, FR-SDK-11, FR-SDK-12, FR-SDK-13, FR-SDK-NEW-1
...
**Then** the handler registrations are significantly shorter (target: <100 lines for handler logic vs ~300 lines in original)
...
- **Git HTTP backend** via `child_process` spawning the `git` binary (git-http-backend for clone/fetch/push, direct git commands for repo init/management)
- **Read-only web UI** mechanically ported from Forgejo's Go HTML templates to EJS/Eta templates, served by Express/Fastify
...
- **Read path**: HTTP request → Express route → git binary (for code/tree/blob) + relay subscription (for issues/PRs) → EJS template → HTML response
...
### Story 3.1: Git HTTP Backend and NIP-34 Handler Service Node

As a **network operator**,
I want a TypeScript git forge built on the SDK that serves git repositories over HTTP and receives NIP-34 events via ILP,
So that repositories are managed through paid Nostr events and cloneable over standard git HTTP protocol.

**Acceptance Criteria:**

**Given** an SDK `createNode()` with handlers registered for NIP-34 kinds (30617, 1617, 1618, 1619, 1621, 1622, 1630-1633)
...
**And** an Express/Fastify HTTP server starts serving the git HTTP backend and web UI
...
**And** repository metadata (name, description, pubkey-owner) is stored in a local metadata file or SQLite
**And** `ctx.accept()` is called with the repository metadata

**Given** an incoming kind:1617 (Patch) event
...
**Given** an incoming kind:1621 (Issue) event
**When** the handler processes it
**Then** `ctx.accept()` is called (the issue is a Nostr event stored on the relay — the Rig just acknowledges receipt)
**And** the web UI will render the issue by querying the relay for events referencing this repository

**Given** a git clone request over HTTP (`GET /repo.git/info/refs?service=git-upload-pack`)
...
### Story 3.2: Nostr Pubkey-Native Git Identity
...
### Story 3.3: Read-Only Code Browsing Web UI (TypeScript Port)

As a **developer**,
I want to browse repositories, view code, diffs, and commit history through a web UI,
So that I can explore projects hosted on the Rig without needing a specialized client.
...
**And** the HTML is rendered from EJS/Eta templates (mechanically ported from Forgejo's Go HTML templates)
...
**Given** the template port scope
**When** compared to Forgejo's full UI
**Then** only read-only code browsing features are ported: repo list, file tree, blob view, commit log, commit diff, blame
**And** NOT ported: admin panels, user settings, OAuth flows, notification system, dashboard, issue/PR write forms

### Story 3.4: NIP-34 Status Events and PR Lifecycle
...
### Story 3.5: Issues and PRs from Nostr Events on Relay
...
### Story 3.6: Publish @crosstown/rig Package
