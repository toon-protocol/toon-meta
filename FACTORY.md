...
| gas-station | pnpm | `parallel-planner-with-review` | eslint / typecheck / vitest / esbuild | Live — scaffolded 2026-08-27, image builds, `:release` published, gate green | — (new repo; the pnpm recipe is proven by relay) | **12th row.** Extracted from `store` (store#105) so the two gas-station kinds get their own funding, security review and box; deployed to the `gas` nanode the same day. Carries store's `.sandcastle/` unchanged. No `eslint-suppressions.json` — the one suppression store had was an unused import, dropped in the move. 0 typecheck debt, 185 tests. Its `deploy/` is the whole box (nginx/TLS + connector + app + certbot + Watchtower), following the reference-implementation shape store adopted in store#103. |
| connector   | npm workspaces | `parallel-planner-with-review` | `cargo` gate on `crates/` plus `npm run lint/typecheck/test --workspaces --if-present`; the hand-ordered `build` prefix (`shared` → `mina-zkapp`) is **gone** — connector ADR 0065 deleted `packages/mina-zkapp` and the only Jest project with it (connector#1205), so `build`/`typecheck` are plain `--workspaces` calls | Live — scaffolded, image builds, dry-run plan proven, **merged agent PR** | [connector#394](https://github.com/toon-protocol/connector/pull/394) (merged) | npm-workspaces variant, done last (most exotic). Sole repo with no root `"type": "module"` → tsx transpiles `.sandcastle/*.ts` to CJS, which broke on top-level `await` (fixed connector#392, `main()` wrapper) and then on `require()`-ing the ESM-only engine (fixed connector#393, nested `.sandcastle/package.json` = `{"type":"module"}` scopes just the runner dir). `npm ci` (no corepack); native deps (libsql/bigint-buffer) build clean in `node:22-bookworm` with zero apt additions — o1js was the reason that mattered and it is gone. 0 typecheck debt. |
...
| gas-station | classic branch protection | `CI OK` | yes | PR required, 0 approvals |
