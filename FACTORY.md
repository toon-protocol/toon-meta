...
whether the same PR is green). Workflow: `.github/workflows/auto-merge.yml`, carrying
toon-meta's own check-suite/review/merged-PR triggers directly (it needs no shim) and, in every
other factory repo, invoked by that repo's `.github/workflows/auto-merge-shim.yml` — fanned out
verbatim from the canonical copy at `scripts/factory/auto-merge-shim.yml` — on the same three
events, plus a 6-hourly safety cron everywhere.
...
Until a repo has the shim, the 6-hourly cron is not that repo's safety net, it is its *only*
trigger: every PR there waits up to 6h after going green, and a PR that settles `BEHIND` after
an `update-branch` can only be re-checked on the next scheduled pass.

The job-level `if:` that filters shim-forwarded events (`check_suite`, `pull_request_review`,
`pull_request`) reads `github.event.*` fields under the assumption that a `workflow_call` invoked
from a repo's shim sees that shim's own triggering payload, not a synthetic `workflow_call`
event. Confirmed against GitHub's docs, not just the workflow's own comment: "When a reusable
workflow is triggered by a caller workflow, the `github` context is always associated with the
caller workflow" (Reusing workflow configurations → `github` context) — so
`github.event.check_suite.head_branch` and friends resolve identically whether the workflow runs
from toon-meta's own trigger or a shim's `uses:` forward.
