...
`store`'s `g.toon.ario` naming in [`docs/deployment.md`](../docs/deployment.md) disagrees with what that box actually terminates (`g.toon.store` and `g.toon.relay.store`) — `ario` is the box label, not the ILP address. Otherwise: image-publish workflow parity across the newer repos, and per-repo `CLAUDE.md` alignment to this folder.
