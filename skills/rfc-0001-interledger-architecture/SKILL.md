...
- **Children** = service nodes under the apex: **relay** (the Nostr relay, pay-per-publish), **store** (NIP-90 compute; the only deployed kind is 5094 Arweave blob storage), **swap** (multi-chain swap peer). Each is registered `relation:'child'` and tags `g.proxy` as its parent.
