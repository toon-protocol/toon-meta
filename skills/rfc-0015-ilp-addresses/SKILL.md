...
- **Child nodes resolve under the apex.** The town relay is `g.proxy.town`; dvm and swap children sit alongside (e.g. `g.proxy.<child>`). A client publishing to the default relay uses destination `g.proxy.town`.
