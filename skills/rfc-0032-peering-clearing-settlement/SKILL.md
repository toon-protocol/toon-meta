...
Implements RFC 0032 semantics with TOON's specific topology: a single **apex** connector (`g.proxy`) and its **child** service nodes (town/store/mill). This skill describes the real peering relations, clearing, and settlement TOON uses.
...
The apex model: the connector is the parent (`g.proxy`); town/store/mill are children. A client is a peer/customer of the apex (pays with claims); the apex forwards to its children for free.
...
- The apex (`g.proxy`) + town/store/mill child topology
