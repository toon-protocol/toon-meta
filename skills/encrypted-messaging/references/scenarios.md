...
> **Why this reference exists:** Agents need step-by-step workflows for common encrypted messaging operations on TOON. Each scenario shows the complete flow from intent to published event, including TOON-specific considerations like encryption overhead and the `client.send()` API. These scenarios bridge the gap between knowing the cryptographic format (nip-spec.md) and knowing the TOON publishing mechanics (toon-extensions.md).
...
- Padding adds overhead. A 10-byte message pads to 32 bytes before encryption. On the relay's flat route that costs nothing extra; on a sloped route such as the blob store it is the padded, sealed size that is metered, never the message you started with.
...
**Why this matters:** Gift wrapping (NIP-59) is the standard way to send private messages on Nostr. It hides the sender identity, content, and timing metadata. On TOON the three-layer envelope makes the event several times larger than the message inside it, and -- on the relay's flat route -- costs no more for that.
...
5. **Send it.** `await client.send({ body: signedGiftWrap })` from `@toon-protocol/client`. The client seals the payload to the terminating connector, reads the route's price, mints the covering claim and carries it -- there is no separate fee-calculation, claim-signing or publish step. Only the kind:1059 gift wrap goes to the relay; the seal and inner event exist only as encrypted content inside it.
...
- On TOON there is no privacy premium. The relay's route (`g.toon.relay`) is flat at 1 base unit of 6-decimal USDC per event, so a gift-wrapped DM and a plaintext kind:1 note of any length cost exactly the same. Metadata protection is free; only the extra bytes are real.
...
1. **Receive the kind:1059 event.** Subscribe using NIP-01 filter `{ kinds: [1059], "#p": ["<your-pubkey>"] }`. The relay's reads speak plain NIP-01, so the `EVENT` message carries a standard JSON object -- parse it as you would from any Nostr relay.
...
- Reading and decrypting gift wraps on TOON is free. No payment is required for subscription or decryption.
...
- The relay speaks plain NIP-01 on reads and returns standard JSON. The only decoding a gift wrap needs is the two NIP-44 decryptions.
...
- On TOON, key rotation has cost implications: re-publishing profile events (kind:0) and re-establishing encrypted conversations are each paid writes at the route's price.
