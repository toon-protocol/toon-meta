...
- **ILP addresses** identify the routing destination. The apex connector is `g.proxy`; child nodes resolve under it — e.g. the relay is `g.proxy.town`, with dvm/swap children alongside. These are hierarchical `g.*` addresses routed by longest-prefix match (see `rfc-0015`). This is what a client puts in a packet's `destination`.
