...
The TOON encoding done by `IlpSpspClient` must match what `decodeEventFromToon()` expects on the receiving end. Use the same `encodeEventToToon()` from `@agent-society/relay` (injected via DI).
...
The `@agent-society/core` package does NOT depend on `@agent-society/relay`. TOON encoding functions must be injected via DI callbacks rather than imported directly:
```typescript
toonEncoder: (event: NostrEvent) => Uint8Array;
toonDecoder: (bytes: Uint8Array) => NostrEvent;
```
