...
- [x] Task 3: Update BLS README with pull command (AC: 8)
  - [x] Add Docker Hub pull command to README Quick Start section:
    ...
    ```
  - [x] Update the existing `docker run` example to use the Docker Hub image name `di3twater/agent-society-bls`
...
- Build command: `docker build -f packages/bls/Dockerfile -t agent-society-bls .` (builds from repo root)
...
- Quick Start section with `docker run` example (currently using `agent-society/bls` — needs update to `di3twater/agent-society-bls`)
- Environment Variables table
- Endpoints documentation

Changes needed: Update image name in Quick Start from `agent-society/bls` to `di3twater/agent-society-bls`, add `docker pull` command, add available tags documentation.
...
- Updated README with `docker pull` command, corrected image name to `di3twater/agent-society-bls`, added Docker Hub tags documentation
...
| 3 | Image published to `di3twater/agent-society-bls` | ✓ | `env.IMAGE_NAME` line 29 |
...
| 8 | README includes pull command | ✓ | `docker pull di3twater/agent-society-bls:latest` in README Quick Start |
