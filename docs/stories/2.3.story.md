...
- [x] Task 1: Create SpspTimeoutError class (AC: 5)
  ...
  - [x] Extend `AgentSocietyError` with code `'SPSP_TIMEOUT'`
...
import { SpspTimeoutError, AgentSocietyError } from './errors.js';
...
1. **SpspTimeoutError** (`errors.ts:55-63`): Well-implemented error class extending AgentSocietyError with proper error code `SPSP_TIMEOUT` and recipientPubkey storage for debugging context.
