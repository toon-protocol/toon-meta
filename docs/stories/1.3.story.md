...
- [x] Task 2: Create custom error class for parsing errors (AC: 5)
  ...
  - [x] Define `AgentSocietyError` base class extending Error with `code` and `cause` properties
  - [x] Define `InvalidEventError` extending `AgentSocietyError` for parsing failures
...
class InvalidEventError extends AgentSocietyError {
...
|------|--------|-------------|
| `packages/core/src/errors.ts` | New | AgentSocietyError base class and InvalidEventError |
