...
**As a** library consumer importing from `@agent-society/core`,
...
**so that** I can discover and use the embedded-mode APIs (`createDirectRuntimeClient`, `createDirectConnectorAdmin`, `createAgentSocietyNode`) through the public package entry point, and HTTP-only consumers continue to work without changes.
...
5. `packages/core/src/index.ts` re-exports all new Epic 10 modules: `createDirectRuntimeClient`, `createDirectConnectorAdmin`, `createHttpRuntimeClient`, `createAgentSocietyNode`, and their associated types
...
- [x] Task 4: Update `core/index.ts` exports (AC: 5)
  - [x] In `packages/core/src/index.ts`:
    ...
    - Add a new section re-exporting from `'./compose.js'`:
      ...
      - Types: `AgentSocietyNodeConfig`, `AgentSocietyNode`, `AgentSocietyNodeStartResult`, `EmbeddableConnectorLike`, `PacketHandler`, `HandlePacketRequest`, `HandlePacketAcceptResponse`, `HandlePacketRejectResponse`, `HandlePacketResponse`
...
├── compose.ts                        # createAgentSocietyNode (Story 10.4, already complete)
├── compose.test.ts
├── types.ts
├── constants.ts
├── errors.ts
...
- `createAgentSocietyNode` (function)
- `AgentSocietyNodeConfig` (type)
- `AgentSocietyNode` (type)
- `AgentSocietyNodeStartResult` (type)
...
- `docker/src/entrypoint.ts` (lines 38, 721, 800): Imports `createAgentRuntimeClient` from `@agent-society/core`. The backward-compat alias keeps this working without changes.
...
5. Updated core/index.ts to re-export all Epic 10 modules: createHttpRuntimeClient, createDirectRuntimeClient, createDirectConnectorAdmin, createAgentSocietyNode, and all associated types
