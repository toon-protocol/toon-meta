...
1. `IlpSendResult` field name aligned between agent-society and agent-runtime — both use the same field name for the success indicator
...
- [x] Task 1: Determine the canonical field name (AC: 1)
  ...
  - [x] Decision: Either update agent-society to use `fulfilled` (match runtime) OR coordinate with agent-runtime to return `accepted` (match society)
...
**Cross-repo coordination:** This fix is one-sided (agent-society only). It makes agent-society tolerant of both field names. A corresponding fix in agent-runtime (returning `accepted` instead of `fulfilled`) could also be done but is not required.
