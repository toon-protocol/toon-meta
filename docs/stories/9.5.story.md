...
1. `npm pack` succeeds for `@agent-society/bls`, `@agent-society/core`, and `@agent-society/relay` without errors
...
5. TypeScript imports resolve correctly in the fresh project: `import { NostrPeerDiscovery } from '@agent-society/core'` compiles without errors
6. The `@agent-society/relay` tarball declares `@agent-society/bls` and `@agent-society/core` as dependencies with `^1.0.0` (not `workspace:*`)
...
- [x] Task 2: Run `npm pack` for each package (AC: 1)
  - [x] Run `npm pack` in `packages/bls/` — produces `agent-society-bls-1.0.0.tgz`
  - [x] Run `npm pack` in `packages/core/` — produces `agent-society-core-1.0.0.tgz`
  - [x] Run `npm pack` in `packages/relay/` — produces `agent-society-relay-1.0.0.tgz`
...
- [x] Task 3: Inspect tarball contents for each package (AC: 2, 3, 6)
  ...
  - [x] **relay**: Inspect `package/package.json` inside tarball — verify `dependencies` has `"@agent-society/bls": "^1.0.0"` and `"@agent-society/core": "^1.0.0"` (not `workspace:*`)

- [x] Task 4: Test install from tarballs in a fresh project (AC: 4, 5)
  - [x] Create a temporary directory outside the workspace (e.g., `/tmp/agent-society-pack-test/`)
  ...
  - [x] Install packages from tarballs: `npm install ./agent-society-bls-1.0.0.tgz ./agent-society-core-1.0.0.tgz ./agent-society-relay-1.0.0.tgz`
...
- [x] Task 5: Clean up tarballs from package directories (AC: 8)
  - [x] Delete `packages/bls/agent-society-bls-1.0.0.tgz`
  - [x] Delete `packages/core/agent-society-core-1.0.0.tgz`
  - [x] Delete `packages/relay/agent-society-relay-1.0.0.tgz`
...
**core** has `@agent-society/relay` as a devDependency (`^1.0.0`) — devDependencies are NOT included in published packages.
**relay** depends on `@agent-society/bls: "^1.0.0"` and `@agent-society/core: "^1.0.0"` in production dependencies.
...
- The tarball is created in the current working directory. For scoped packages, npm strips the `@` and replaces `/` with `-`, so `@agent-society/bls@1.0.0` becomes `agent-society-bls-1.0.0.tgz`
...
```
packages/
├── bls/            # @agent-society/bls — npm pack here
│   ├── dist/       # Build output (must exist before pack)
│   ├── README.md   # Included in tarball automatically
│   └── package.json
├── core/           # @agent-society/core — npm pack here
│   ├── dist/
│   ├── README.md
│   └── package.json
└── relay/          # @agent-society/relay — npm pack here
    ├── dist/
    ├── README.md
    └── package.json
```
...
4. Relay tarball package.json confirms `"@agent-society/bls": "^1.0.0"` and `"@agent-society/core": "^1.0.0"` (not workspace:*)
5. Fresh project install (`npm install ./agent-society-*.tgz`) succeeds — 822 transitive packages installed
