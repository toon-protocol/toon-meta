...
- [x] Task 2: Create PeerDiscoveryError class (AC: 4)
  ...
  - [x] Error should extend `AgentSocietyError` with code `PEER_DISCOVERY_FAILED`
...
- `AgentSocietyError` base class exists in `packages/core/src/errors.ts` with `code` and `cause` properties
