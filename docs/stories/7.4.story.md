...
- [x] Task 1: Add `spspMinPrice` field to `BlsConfig` and local SPSP kind constant in `packages/bls/src/bls/types.ts` (AC: 1, 3)
  - [x]Add `export const SPSP_REQUEST_KIND = 23194;` constant (BLS does not import from `@agent-society/core`, so define locally)
...
- The `@agent-society/core` package has the SPSP event kind constants: `SPSP_REQUEST_KIND = 23194` and `SPSP_RESPONSE_KIND = 23195` in `packages/core/src/constants.ts`
- However, the BLS packages (`packages/bls` and `packages/relay`) do NOT import from `@agent-society/core` — they use raw kind numbers. The BLS treats events generically (any Nostr event kind). For this story, use the literal `23194` in BLS code rather than importing the constant from core.
...
**SPSP event kind:**
The SPSP request event kind is `23194` (defined as `SPSP_REQUEST_KIND` in `packages/core/src/constants.ts`). The BLS packages do not depend on `@agent-society/core`, so the literal number `23194` should be used. Consider defining a local constant `const SPSP_REQUEST_KIND = 23194` in `types.ts` for clarity.
