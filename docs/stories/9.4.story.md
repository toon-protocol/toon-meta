...
1. `packages/relay/package.json` dependencies list `"@agent-society/bls": "^1.0.0"` (was `workspace:*`)
2. `packages/relay/package.json` dependencies list `"@agent-society/core": "^1.0.0"` (was `workspace:*`)
3. `packages/core/package.json` devDependencies list `"@agent-society/relay": "^1.0.0"` (was `workspace:*`)
...
- [x] Task 1: Update `@agent-society/relay` production dependencies (AC: 1, 2, 5)
  - [x] Open `packages/relay/package.json`
  - [x] Change `"@agent-society/bls": "workspace:*"` to `"@agent-society/bls": "^1.0.0"` (line 39)
  - [x] Change `"@agent-society/core": "workspace:*"` to `"@agent-society/core": "^1.0.0"` (line 40)
  - [x] Verify no other fields were modified

- [x] Task 2: Update `@agent-society/core` dev dependencies (AC: 3, 5)
  - [x] Open `packages/core/package.json`
  - [x] Change `"@agent-society/relay": "workspace:*"` to `"@agent-society/relay": "^1.0.0"` (line 45)
  - [x] Verify no other fields were modified

- [x] Task 3: Verify private packages retain `workspace:*` (AC: 4)
  - [x] Confirm `packages/examples/package.json` still has `"@agent-society/core": "workspace:*"` and `"@agent-society/relay": "workspace:*"` in dependencies
  - [x] Confirm `docker/package.json` still has `"@agent-society/core": "workspace:*"` and `"@agent-society/relay": "workspace:*"` in dependencies
...
**Private packages keep `workspace:*`:** `@agent-society/examples` and `@agent-society/docker` are `"private": true` and will never be published. There is no reason to change their references — `workspace:*` is correct for private, workspace-only packages.
...
|---------|-------|---------|--------|
| `packages/relay/package.json` | `dependencies["@agent-society/bls"]` | `workspace:*` | `^1.0.0` |
| `packages/relay/package.json` | `dependencies["@agent-society/core"]` | `workspace:*` | `^1.0.0` |
| `packages/core/package.json` | `devDependencies["@agent-society/relay"]` | `workspace:*` | `^1.0.0` |

**Private packages (DO NOT change):**
| Package | Field | Value | Reason |
|---------|-------|-------|--------|
| `packages/examples/package.json` | `dependencies["@agent-society/core"]` | `workspace:*` | `"private": true` — never published |
| `packages/examples/package.json` | `dependencies["@agent-society/relay"]` | `workspace:*` | `"private": true` — never published |
| `docker/package.json` | `dependencies["@agent-society/core"]` | `workspace:*` | `"private": true` — never published |
| `docker/package.json` | `dependencies["@agent-society/relay"]` | `workspace:*` | `"private": true` — never published |
...
```
packages/
├── core/     # @agent-society/core — change devDependencies
├── bls/      # @agent-society/bls — no workspace refs, no changes
├── relay/    # @agent-society/relay — change dependencies
└── examples/ # @agent-society/examples — private, no changes
docker/       # @agent-society/docker — private, no changes
```
...
|------|--------|-------------|
| `packages/relay/package.json` | Modified | Changed `@agent-society/bls` and `@agent-society/core` from `workspace:*` to `^1.0.0` |
| `packages/core/package.json` | Modified | Changed `@agent-society/relay` devDependency from `workspace:*` to `^1.0.0` |
...
The diff is clean: `packages/relay/package.json` has `@agent-society/bls` and `@agent-society/core` changed from `workspace:*` to `^1.0.0` in `dependencies`, and `packages/core/package.json` has `@agent-society/relay` changed from `workspace:*` to `^1.0.0` in `devDependencies`. No other fields were modified.
...
| AC | Status | Evidence |
|----|--------|----------|
| 1. relay/package.json `@agent-society/bls`: `^1.0.0` | ✓ | Verified at line 39 |
| 2. relay/package.json `@agent-society/core`: `^1.0.0` | ✓ | Verified at line 40 |
| 3. core/package.json devDeps `@agent-society/relay`: `^1.0.0` | ✓ | Verified at line 45 |
