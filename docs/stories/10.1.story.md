...
12. **`docker/src/entrypoint.ts`** — Imports `HandlePaymentRequest`, `HandlePaymentAcceptResponse`, `HandlePaymentRejectResponse` from `@agent-society/bls` (lines 65-67). Route `'/handle-payment'` at line 360. Multiple type usages throughout the `/handle-payment` handler (lines 360-555).
...
- Build validation: `@agent-society/core`, `@agent-society/bls`, `@agent-society/relay` all build clean. Docker type-checks clean.
