...
- [x] Task 2: Update `createConnectorAdminClient()` to match `ConnectorAdminClient` interface (AC: 1)
  - [x] Refactor `createConnectorAdminClient()` to return an object matching the `ConnectorAdminClient` interface from `@agent-society/core`:
  ...
  - [x] Import `ConnectorAdminClient` type from `@agent-society/core` for type safety

- [x] Task 3: Rewire `main()` bootstrap flow to use `BootstrapService` with layered discovery (AC: 1, 3)
  - [x] Update imports from `@agent-society/core`: keep `BootstrapService`, `NostrSpspServer`, `buildSpspResponseEvent`, `buildIlpPeerInfoEvent`, `parseSpspRequest`, `type IlpPeerInfo`, `type SpspInfo`, `SPSP_REQUEST_KIND`; add `SocialPeerDiscovery`, `type ConnectorAdminClient`; remove `type KnownPeer` if no longer used in Config
...
- [x] Task 6: Verify startup log sequence (AC: 3)
  - [x] Ensure startup logs include in order:
    ...
    5. "Agent Society Container Ready" banner
...
- **AC 3** (Startup logs): ✓ Log sequence: config summary → service init → bootstrap results → social discovery started → "Agent Society Container Ready" banner
