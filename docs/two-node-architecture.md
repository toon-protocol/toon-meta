...
> **Reconciled, as of 2026-08-08.** All three items flagged here on 2026-08-07 are now settled. They are kept, resolved, rather than deleted, because two were owner decisions and the record of which way they went is load-bearing for the runbook tickets that cite this section.
>
> 1. **The faucet's destination is settled: its own Linode, no connector.** §4 stands as written. The contest with connector PR [#892](https://github.com/toon-protocol/connector/pull/892) is gone — #892 has been corrected and now excludes the faucet explicitly ("the faucet gets its OWN box, with no connector on it"), [connector#870](https://github.com/toon-protocol/connector/issues/870) was rescoped to match, and the move is tracked by [connector#898](https://github.com/toon-protocol/connector/issues/898). [toon-meta#310](https://github.com/toon-protocol/toon-meta/issues/310)'s "faucet moves to the relay box" bullet has been corrected to agree. **Consequence for the runbook:** the faucet lives on the apex today, so #898 is a *precondition* of the teardown rather than a step of it — destroying the apex first takes the faucet with it. [toon-meta#313](https://github.com/toon-protocol/toon-meta/issues/313) now blocks on #898.
> 2. **The genesis seed's pubkey does not change.** The relay box adopts box 1's announcer identity (`30fdd01d…`) — the exact pubkey the committed seed already pins — so that row drops out of §5.3 and only `ilpAddress` and `btpEndpoint` change. This is now the decided plan rather than a possibility, specified as an acceptance criterion of [connector#870](https://github.com/toon-protocol/connector/issues/870) and as the operator runbook in [toon-meta#311](https://github.com/toon-protocol/toon-meta/issues/311); #892 is its repo half and is still open. **Consequence, and it is not obvious:** adoption means two publishers can hold one identity, and `kind:10032` is regular-replaceable by `(pubkey, kind)` with no `d` tag. The apex's sidecar and the relay box's loop must therefore never run concurrently, and no verification step may identify a publisher by its author. §6's ordering and `docs/operators/announcing-a-node.md`'s cutover guidance were both written for *different*-key cutovers, where overlap is the safe direction to be wrong in; under a shared key it is the outage. #311 is the runbook of record.
> 3. **§6.2 step 10's repo half has landed.** Connector PR [#891](https://github.com/toon-protocol/connector/pull/891) merged 2026-08-08, repointing the store box's `[announce] publish_btp_url` from `wss://proxy.devnet.toonprotocol.dev/ilp/btp` to `wss://proxy.relay.devnet.toonprotocol.dev/ilp/btp`. §6.1's producer table and §6.2 step 10 still describe the **live box**, which the repo change does not touch (the bind-mounted file leads). Step 10 remains a human step, tracked by [toon-meta#312](https://github.com/toon-protocol/toon-meta/issues/312).
...
> |---|---|
> | `token_network = "0xa79C3b1dbcEA00a6d84735a134395D8eF6D6a478"` (§1.3, §3.3) | **Retired.** That pair went out with the ADR 0059 cutover, broadcast 2026-08-28 at block 46055303. Live: registry `0x0c41D9D424d6B075A3cEa1068a694f7847a8CCa5`, TokenNetwork `0x1B4606218ceE5Bf02B546e416905F4D3FC8a0249`. And a connector is configured with the **registry** — it resolves the TokenNetwork at boot via `getTokenNetwork(token)`, so the TokenNetwork value is **derived**, never pinned independently. |
...
| `packages/core/src/discovery/genesis-peers.json:3` | `"pubkey": "30fdd01d…"` | **no change** — the relay box *adopts* this identity rather than announcing under its own (see the reconciliation note, item 2) |
