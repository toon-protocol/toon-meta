...
> `docs/314-two-node-architecture` working branch — the **pre-reconciliation** copy dated
> 2026-08-07. `main` has since moved: [#325](https://github.com/toon-protocol/toon-meta/pull/325)
> replaced that "Unreconciled" preamble with a "Reconciled, as of 2026-08-08" one, settling all
> three flagged items in the same direction this audit independently confirms. **Finding 3 below
> is therefore already folded into `main`'s copy** and is retained as corroboration, not as news.
> Findings 1 and 2 are not in `main`'s copy and are the reason this document exists. Every
> section reference below still resolves — #325 changed only the preamble and one row of §5.3's
> table (the seed `pubkey`, which this audit does not rely on).
