# RFC: Payment Termination — TOON in front of any HTTP service

**Status:** Draft · **Scope:** architecture / direction · **Audience:** connector + client + ecosystem

## The idea, in one line

Terminate **payment** at the edge the way nginx terminates **TLS**. An agent speaks the payment
protocol to a reverse proxy; the proxy validates and settles, then forwards a clean, byte-identical
HTTP request to a backend the developer wrote with **zero payment awareness**.

```
nginx:  client ──HTTPS──► nginx ──plain HTTP──► app   (app never sees TLS)
TOON:   agent ──paid────► terminator ──plain HTTP──► app   (app never sees payment)
```
...
keep their existing HTTP service and put a terminator in front of it. Termination is *why TLS is
everywhere*; the same decoupling is the play for payment.
...
**Rung 1 — Onboard via x402 / HTTP 402.** The terminator answers an unpaid request with `402`
...
the claim rides in the `ILP-Payment-Channel-Claim` header. The terminator validates the claim,
...
```
Agent → GET https://api.foo/v1/x ───────────────► Terminator (= connector + proxy handler)
Terminator → 402  accepts:[ exact(base-usdc), toon-channel(ilpAddress, /ilp) ]
...
Terminator → validate claim (ClaimReceiver) ─ strip hop-by-hop hdrs ─ inject X-TOON-* ─ replay ─►
                                                                          Backend (plain HTTP, oblivious)
Agent ◄── 200  body: OER ILP FULFILL (data = raw HTTP response) ◄── Terminator ◄── Backend response
```
...
1. **`data` is opaque arbitrary bytes**, so it carries a raw HTTP request as easily as a TOON
   ...
   (`shallowParseToon` → kind dispatch) used by the **relay/store** apps. The **terminator is a
...
3. **Gift-wrap means the CLAIM, not the data.** The connector-level NIP-59 claim wrap encrypts the
   ...
   + intermediary privacy); it is optional/config-gated and orthogonal to `data`. The terminator
   does **not** use the SDK event-wrap (which stuffs a wrapped Nostr event *into* `data`) — that's
   swap-app behavior, and the terminator has no event to wrap.
...
This invariant is what keeps the relay (a TOON-event app) and the terminator (an HTTP app) as peers
...
principle. Per-byte pricing (TOON's default for writes) is wrong for terminated APIs, because the
...
— sign method + path + body-digest + price so the terminator verifies the claim is for this exact
call. This pulls 9421 onto the **critical path** (it is currently filed under egress, see *What's
built*).

Trust terminates at the edge, exactly as with TLS: the backend trusts injected headers
(`X-TOON-Payer` pubkey, `X-TOON-Amount`, `X-TOON-Chain`) over a private hop. This realizes the
narrative's **"payment is authentication, no API keys"** — the terminator hands the backend a
verified payer identity.

## Deployment topologies

The terminator is an **edge concern — not one connector per app**, just as one nginx fronts many
vhosts.

- **Shared ingress terminator (default).** One connector fronts many backends by host/path; backend
...
- **Sidecar terminator (sovereignty).** An app co-runs its own connector (the Envoy-sidecar
  analogue); the developer holds their own channels/keys and earns directly.
- **Managed edge ("Cloudflare for agent payments").** A third party runs the terminator network;
...
(like the terminator's HTTP app), co-located in the hub's Compose bundle but a separate service.
...
So a **pure payment terminator = connector + proxy handler + your HTTP app**: zero Nostr, zero relay
in the path; peers set statically or via the admin API. **For the terminator, discovery is the
...
1. **Static config at boot** — `peers[]` (id, url, authToken, chain,
   ...
   grammar exposes (`peer {} / route {} / upstream {}`, where `upstream` is the terminator's
   backend).
...
The connector is the HTTP front; real nginx, if present, is only a dumb TLS terminator routing
...
An agent opens **one** channel with its local hub and reaches any destination over N hops; it needs
...
follow-on spec; it is **not** terminator MVP.
...
- the **x402 greeting** (the dual-entry 402) at the terminator edge;
