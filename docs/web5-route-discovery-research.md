...
> "A unique property of DID DHT is its dependence on a single non-rotatable key, which we refer to as an _Identity Key
> Pair_ … This requirement stems from BEP44" — quoting BEP44: "To authenticate that only the original publisher can update
> an item, it is signed by a private key generated by the original publisher." — `[did-dht §Identity Key Pair]`

> "Consequently, DHT records, including DID DHT Documents, are _independently verifiable_. This independence implies that
...
two hours by the holder or a PoW-paying gateway. `[adr-0050]` dropped `ttl_secs` on the reasoning that "a _pushed_ copy
