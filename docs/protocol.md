...
#### Swap node recipient key discovery

A `swap` node (the multi-chain swap peer) is the one case where the kind:10032
`pubkey` is operationally load-bearing for clients beyond peering. A
`streamSwap` caller must NIP-59 **gift-wrap** the swap request to the swap node, and
the recipient pubkey it encrypts to (passed as `millPubkey`) is exactly the
`pubkey` advertised in the swap node's kind:10032 IlpPeerInfo event.

That key is derived from the swap node's **`MILL_MNEMONIC`** (the operator's BIP-39
identity), via `fromMnemonic(mnemonic)`. It is the SAME identity the swap node uses
...
> the swap node's kind:10032 `pubkey` (or the `swapRecipientPubkey` field of the
> swap node's `mill_ready` startup log) — never from `NODE_NOSTR_SECRET_KEY`.
