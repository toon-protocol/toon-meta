...
| Rescue (unilateral exit, no counterparty cooperation) | `closeChannel` + `settleChannel` callable by a single participant with no signature from the other side — exercised live in the [2026-07-31 apex identity rotation](./operators/2026-07-31-apex-settlement-identity-rotation.md), where eight channels were closed/settled by counterparties alone | equivalent unilateral close/settle instruction on the program | equivalent unilateral path on the zkApp |
...
soaking just because its instrument is newer — but the Mina row carries a
prerequisite the other two have already cleared.
...
- Whether any family has *met* the bar in §3 is not decided here and stays
  a human call, per this ticket's explicit scope.
