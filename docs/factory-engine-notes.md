...
- **Symptom:** a contracts CI gate goes red on a PR with no product cause —
  ...
  already APPROVED. The failure message does not name the toolchain, so nothing at the
  call site hints that the runner just picked up a new nightly build.
- **Cause:** `contracts.yml`'s `foundry-toolchain` install floated `nightly`. Forge
  `1.8.0-nightly` regressed the ERC-2771 fork test — reproduced identically in
  `ghcr.io/foundry-rs/foundry:nightly` — while stable **v1.7.1** passed on the
  identical branch against the identical public RPC (`https://sepolia.base.org`), with
  the live distributor still holding ~100M USDC. No chain-state or PR cause; the
...
- **Fix:** pin every `foundry-toolchain` install in the workflow to a specific stable
  ...
  that motivated the pin. The general lesson: a CI gate that exists to prove
  chain/deploy code works must pin its toolchain to a stable version — a floating
  `nightly` channel can turn the gate red across the whole fleet with no code change
  to bisect, and the error it produces won't name the toolchain as the cause.
