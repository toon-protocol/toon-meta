...
2. **Custom org GitHub App** (needed to write org-level Projects — the default
   ...
   - Install on the 7 repos; add `APP_ID` and `APP_PRIVATE_KEY` as org secrets.
...
Per repo, stagger the Loop A `cron` minute/hour so all 7 don't fire at once and
spike Max usage.

## Graduation (don't enable everything at once)

1. Land **only `backlog-manager.yml` in dry-run** on all 7; review reports for a
...
5. Template the proven set across the remaining 6.

## Rate-limit & safety notes

- 7 repos × 4 loops share **one** Max limit. Keep Loop A **daily** (not hourly),
